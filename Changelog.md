...
- Add (wile-c ...) special form, and use it in regexp.scm: using it to
  eliminate repeated strlen() calls saves over 25% of the runtime to
  generate the scheme lexer in the increasingly misnamed mini-lex-gen.

- Add matrix-matrix-multiply and matrix-print to library,
  useful for linear algebra.

- Add bytevector-hash-64, bytevector-and!, bytevector-or!, bytevector-xor!
  functions; tweak wile-build-info function to remove flag argument

- Change (almost?) all multi-line emit-code to use here-docs: it's a trivial
  expansion of the code in exchange for better readability and modifiability.

- Make stack trace able to handle foo.constprop.N as generated by gcc.

- Improve def-struct macro to allow array sub-fields:
  (def-struct foo bar1 (bar2 3) bar3 (bar4 15))

- Turn write-string into an inlined bit of code rather than a function call;
  that speeds it up somewhat noticeably. Add explicit configuration file to
  build-stages to override any env.var etc.

- Add regexp code to repo

- Add real gprof profiling

- Tweak lexer to allow indentable HEREDOCs. Allow spaces in here-doc
  start tag, as in "#<< FOO". Add test for here-docs to wtest. Tweak
  here-doc scanner to remove the last newline, it's easier to add one
  more line than to remove one in scheme.

- Stage 1 of improving stack traces: the user-invoked version points at
  the scheme sources now when appropriate, the stack_trace_minimal version
  does not yet do that.
  Small tweaks to stack-trace output: reverse order

- Fix an off-by-one bug in string-copy.

- Fix a stack friendliness inadequacy in list-group-by, incidentally find
  and fix a bug in same. Add test for list-group-by.

- Add detection of merge conflict markers to lexer.

- Fix quoting issue which caused quine to fail
