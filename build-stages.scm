...
(define (get-md5 file)
  (let* ((port (run-read-command (string-append "md5sum " file)))
	 (line (read-line port)))
    (close-port port)
    (car (string-split-by char-whitespace? line))))
...
(make-stage 1)
(set! m1x (get-md5 "wilec.stage1"))
(set! m1c (get-md5 "wilec.stage1.c"))
(set! m1l (get-md5 "libwrtl.stage1.a"))
(make-stage 2)
(set! m2x (get-md5 "wilec.stage2"))
(set! m2c (get-md5 "wilec.stage2.c"))
(set! m2l (get-md5 "libwrtl.stage2.a"))
...
(if (and (string=? m1x m2x)
    ...
    (begin
      ...
      (run-cmd-or-die #f "md5sum wilec.stage[12].c")
      (run-cmd-or-die #f "md5sum wilec.stage[12]")
      (run-cmd-or-die #f "md5sum wilec-dbg.stage[12]")
      (run-cmd-or-die #f "md5sum libwrtl.stage[12].a")
      (run-cmd-or-die #f "md5sum libwrtl-dbg.stage[12].a")
